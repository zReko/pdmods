...
local mpath=ModPath
Hooks:Add("LocalizationManagerPostInit", "LocManInit:preplanning_extracted", function(loc)
	local path="loc.en.txt"
...
if requiredScript == "lib/managers/menumanager" then
	...
	function MenuPrePlanningInitiator:modifiy_node_preplanning(node, ...)
		...
		self:create_divider(node, "title_preplanning_manager", managers.localization:text("wolfhud_extracted_preplanning_manager_title"), nil, tweak_data.screen_colors.text)
		...
		local save_params = {
			...
			text_id = "wolfhud_extracted_preplanning_save",
			callback = "save_preplanning",
			tooltip = {
				name = managers.localization:text("wolfhud_extracted_preplanning_save_tooltip_title"),
				desc = managers.localization:text("wolfhud_extracted_preplanning_save_tooltip_desc"),
		...
		local load_params = {
			...
			text_id = "wolfhud_extracted_preplanning_load",
			callback = "set_load_mode",
			next_node = PrePlanningManager.saved_plans_node,
			tooltip = {
				name = managers.localization:text("wolfhud_extracted_preplanning_load_tooltip_title"),
				desc = managers.localization:text("wolfhud_extracted_preplanning_load_tooltip_desc"),
		...
		local delete_params = {
			...
			text_id = "wolfhud_extracted_preplanning_delete",
			callback = "set_delete_mode",
			next_node = PrePlanningManager.saved_plans_node,
			tooltip = {
				name = managers.localization:text("wolfhud_extracted_preplanning_delete_tooltip_title"),
				desc = managers.localization:text("wolfhud_extracted_preplanning_delete_tooltip_desc"),
		...
		local reset_params = {
			...
			text_id = "wolfhud_extracted_preplanning_reset",
			callback = "reset_preplanning",
			tooltip = {
				name = managers.localization:text("wolfhud_extracted_preplanning_reset_tooltip_title"),
				desc = managers.localization:text("wolfhud_extracted_preplanning_reset_tooltip_desc"),
				texture = "guis/dlcs/big_bank/textures/pd2/pre_planning/drawtools_atlas",
		...
		local reset_all_params = {
			...
			text_id = "wolfhud_extracted_preplanning_reset_all",
			callback = "full_reset_preplanning",
			tooltip = {
				name = managers.localization:text("wolfhud_extracted_preplanning_reset_all_tooltip_title"),
				desc = managers.localization:text("wolfhud_extracted_preplanning_reset_all_tooltip_desc"),
				texture = "guis/dlcs/big_bank/textures/pd2/pre_planning/drawtools_atlas",
				texture_rect = {30, 0, 29, 29},
				errors = {},
			},
	...
	function MenuPrePlanningInitiator:modifiy_node_preplanning_saved_plans(node, item_name, selected_item)
		...
		if table.size(saved_plans) <= 0 then
			self:create_divider(node, "title_category_saved_plans", managers.localization:text("wolfhud_extracted_preplanning_error_no_saved_plans"), nil, tweak_data.screen_colors.text)
			...
			if PrePlanningManager._PREPLANNING_DELETE_MODE then
				self:create_divider(node, "title_category_saved_plans", managers.localization:text("wolfhud_extracted_preplanning_delete"), nil, tweak_data.screen_colors.text)
				selected_item = nil
			else
				self:create_divider(node, "title_category_saved_plans", managers.localization:text("wolfhud_extracted_preplanning_load"), nil, tweak_data.screen_colors.text)
			end
...
elseif requiredScript == "lib/managers/preplanningmanager" then
		...
		function PrePlanningManager.save_plans()
			if not PrePlanningManager.DirectoryExists(PrePlanningManager._SAVE_FOLDER) then
				if not PrePlanningManager.createDirectory(PrePlanningManager._SAVE_FOLDER) then
					managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_folder_creation_failed", { FOLDER = PrePlanningManager._SAVE_FOLDER }, true)
				end
			end
			...
			if file then
				...
				managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_file_creation_failed", { FILE = PrePlanningManager._SAVE_FILE }, true)
			end
		end
	...
	function PrePlanningManager:save_preplanning()
		...
		if not PrePlanningManager.has_free_save_slot() then
			managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_no_save_slot", {}, true)
		elseif not PrePlanningManager.has_current_plan() then
			managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_no_current_plan", {}, true)
		else
			local default_name = ""
			...
			local menu_options = {
				[1] = {
					text = managers.localization:text("wolfhud_extracted_dialog_save"),
			...
			QuickInputMenu:new(managers.localization:text("wolfhud_extracted_preplanning_dialog_save"), managers.localization:text("wolfhud_extracted_preplanning_dialog_save_desc", {PLAN = plan_str}), default_name, menu_options, true)
		end
	end
	...
	function PrePlanningManager:save_preplanning_clbk(name)
		if name and name ~= "" then
			...
			if PrePlanningManager.save_plans() then
				managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_saved_success", {}, false)
			else
	...
	function PrePlanningManager:load_preplanning(name)
		if PrePlanningManager._SAVED_PLANS and PrePlanningManager._SAVED_PLANS[name] then
			...
			if something_loaded and not (missing_skill or missing_favours or missing_money) then
				managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_loaded_success", {}, false)
			elseif something_loaded then
				local error_msg = ""
				if missing_skill then
					error_msg = managers.localization:text("wolfhud_extracted_preplanning_msg_loaded_missing_skill")
				end
				if missing_favours then
					error_msg = string.format("%s%s %s", error_msg, (missing_skill and ";" or ""), managers.localization:text("wolfhud_extracted_preplanning_msg_loaded_missing_favours"))
				end
				if missing_money then
					error_msg = string.format("%s%s %s", error_msg, ((missing_skill or missing_favours) and ";" or ""), managers.localization:text("wolfhud_extracted_preplanning_msg_loaded_missing_money"))
				end
				managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_loaded_partitially", {ERRORMSG = error_msg}, true)
            else
				managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_loaded_failed", {}, true)
			end
		end
	end
	...
	function PrePlanningManager:reset_preplanning(full_reset)
		if not PrePlanningManager.has_current_plan() then
			...
			managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_reseted_success", {}, true)
		end
	end
	...
	function PrePlanningManager:delete_preplanning(name)
		...
		QuickMenu:new( managers.localization:text("wolfhud_extracted_preplanning_dialog_delete"), managers.localization:text("wolfhud_extracted_preplanning_dialog_delete_desc", {NAME = name}), menu_options, true )
	end
	...
	function PrePlanningManager:delete_preplanning_clbk(name)
		if PrePlanningManager._SAVED_PLANS and PrePlanningManager._SAVED_PLANS[name] then
			PrePlanningManager._SAVED_PLANS[name] = nil
			if PrePlanningManager.save_plans() then
				managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_deleted_success", {}, false)
				return
			end
		end
		managers.preplanning:notify_user("wolfhud_extracted_preplanning_msg_deleted_failed", {}, true)
	end
	...
	function PrePlanningManager:current_assets_name()
		...
		if table.size(current_votes) > 0 or table.size(default_votes) > 0 then
			text = string.format("%s%s\n", text, managers.localization:text("wolfhud_extracted_preplanning_votes_title"))
		...
		if table.size(current_assets) > 0 then
			text = string.format("%s%s\n", text, managers.localization:text("wolfhud_extracted_preplanning_assets_title"))
	...
	function PrePlanningManager:notify_user(text_id, macros, show_SP)
		...
		elseif show_SP then
			QuickMenu:new(managers.localization:text("wolfhud_extracted_preplanning_manager_title"), message, { text = managers.localization:text("dialog_ok"), is_cancel_button = true }, true)
		end
	end
end
